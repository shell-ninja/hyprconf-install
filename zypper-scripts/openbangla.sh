...
# Base devel pattern for openSUSE
if ! sudo zypper se -i patterns-devel-base-devel_basis &> /dev/null && ! sudo zypper se -i devel_basis &> /dev/null; then
    msg act "Installing devel_basis pattern..."
    sudo zypper in -y -t pattern devel_basis 2>&1 | tee -a "$log"
fi
